...
class Follower(Node):
    ...
    def bbox_callback(self, msg):
        ...
        for box in msg.bounding_boxes:
            if box.class_id == self.target_class and box.probability >= 0.8:
                # self.get_logger().info(str(box.class_id) + str(box.id))
                self.target_box.append([box.xmin, box.ymin, box.xmax, box.ymax, box.probability])
  ...
  def control_callback(self):
      try:
          # 대상이 없거나 모든 대상의 거리가 7미터 초과인 경우 로봇을 정지
          if not self.target_depth or all(depth > 7.0 for depth in self.target_depth):
